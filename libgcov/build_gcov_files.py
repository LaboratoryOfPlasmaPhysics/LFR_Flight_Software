        ...
        for line in gcov.readlines():
            ...
            if dest_file not in files:
                ...
                if head == '_GCOV_':
                    ...
                    if os.path.exists(gcno_file) and not os.path.exists(gcno_file.replace(args.remove_prefix, args.output_folder)):
                    ...
                    with open(dest_file,'wb') as gcda_file:
                        stamp = bytes(reversed(open(gcno_file, 'rb').read(12)[-4:]))
                        gcda_content = bytearray([int(''.join(value),16) for value in zip(data[::2],data[1::2]) ])
                        gcda_content[8:12]=stamp
                        gcda_file.write(bytes(gcda_content))
                else:
                    raise
