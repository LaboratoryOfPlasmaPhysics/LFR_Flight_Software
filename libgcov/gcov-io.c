...
/* retrieve the id of the CPU the program is running on */
    ...
    for (gi_ptr = gcov_list; gi_ptr; gi_ptr = gi_ptr->next)
    {
        ...
        {
            fi_ptr
                = (const struct gcov_fn_info*)((const char*)gi_ptr->functions + f_ix * fi_stride);
...
static void gcov_write_block(unsigned size)
{
    ...
    for (i = 0; i < size; i++)
    {
        printf("%02X%02X%02X%02X", (unsigned int)(buffer[0]), (unsigned int)(buffer[1]),
...
GCOV_LINKAGE void gcov_write_tag_length(gcov_unsigned_t tag, gcov_unsigned_t length)
{
    gcov_unsigned_t* buffer = gcov_write_words(2);
